...
interface SgKaufMethodsLocal extends SgKaufMethods {
  ...
  _addBuy: (
    newBuy: BuyInfo,
    storedDate: DetailedDateInfo | undefined,
    storedBuy: BuyInfo | undefined
  ) => void;
...
const methods: SgKaufMethodsLocal = {
  ...
  setActiveDate(newDate: string): boolean {
    const dateToSelect = state.shoppingDates.find(
        ...
        if (newArray) {
          ...
          if (newArray.length) {
            ...
            const indexOfDateToDelete =
              state.shoppingDates.indexOf(existingShoppingDate);
    ...
    getProductNames()
      .then((productNames: string[]) => {
        if (!productNames || !productNames.length) {
          console.error(
            "Product names should be an Array of Strings. Got no products. Returning..."
          );
          return false;
        }
    ...
    getProductDescriptions()
      .then((productDescriptions: string[]) => {
        if (!productDescriptions || !productDescriptions.length) {
          console.error(
            "Product descriptions should be an Array of Strings. Got no descriptions. Returning..."
          );
          return false;
        }
    ...
    getProductDefaults()
      .then((productDefaults: Array<string | Product>) => {
        if (!productDefaults || !productDefaults.length) {
          console.log(
            "Product defaults should be an Array of product names or objects. Got no product defaults. Returning..."
          );
          return false;
        }
  ...
  saveProduct(
    ...
    let { name, price, weightAmount, measure, description, discount } =
      productInfo;

    const dateToAddProductTo = state.shoppingDates.find(
    ...
    const buyToAddProductTo = dateToAddProductTo?.buys?.find(
      (buy) => buy.time === time
    );
    ...
    const existingProducts = (buyToAddProductTo.products =
      buyToAddProductTo.products || []);

    if (find(existingProducts, productInfo)) {
      console.warn(
        `Product array already has such a product: ${productInfo.name}. Product will NOT be added. Returning...`
      );
      return methods._breakPromiseExecution();
    }
  ...
  removeProduct(date: string, time: string, productInfoForRemove: Product) {
    ...
    return removeProduct(url)
      .then((data) => {
        ...
        const dateToRemoveProductFrom = state.shoppingDates?.find(
          (shoppingDate) => shoppingDate.date === date
        );
  ...
  getProductTimelineInfo(productRequestInfo: ProductTimelineRequestInfo) {
    ...
    if (!(name && measure && shopName)) {
      ...
      console.log(
        `Provided data: name: ${name}, measure: ${measure}, shopName: ${shopName}`
      );
      return methods._breakPromiseExecution();
    }
